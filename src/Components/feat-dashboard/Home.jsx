...
import AllActivity from "../feat-activity/AllActivity";
...
const Home = () => {
  ...
  const getColumnsChartConfig = (currentConfig, days) => {
    ...
    for (let dayIndex = 0; dayIndex < daysOfWeek.length; dayIndex++) {
        for (let activityIndex = 0; activityIndex < allActivitiesType.length; activityIndex++) {
                series[activityIndex].data[dayIndex] = 
                    days[dayIndex].activities
                        .find(activity => activity.type === allActivitiesType[activityIndex])?.time;
            }
    }
  ...
  useEffect(() => {
    const getUserDashboardActivities = async () => {
        try {
          const response = await axios.get("http://127.0.0.1:3000/get-dashboard-activities");
          ...
          if (response.status === 200) {
            ...
            setDailyActivitiesConfig(getPieChartConfig(dailyActivitiesConfig,response.data.donutDailyActivities));
            setAllActivitiesConfig(getPieChartConfig(allActivitiesConfig,response.data.donutAllActivities));
            setWeeklyActivitiesConfig(getPieChartConfig(weeklyActivitiesConfig, response.data.donutWeeklyActivities));
            setWeeklyColumnsChartConfig(getColumnsChartConfig(weeklyColumnsChartConfig, response.data.columnsWeeklyActivities))
  ...
  return (
    <NavbarDesktop>
      ...
      <div className="grow bg-white">
        ...
        <ReactApexChart options={weeklyColumnsChartConfig.options} series={weeklyColumnsChartConfig.series} type="bar" height={350} />

        </div>

        {dashboardActivities?.allActivities?.map((activity) => <div>{activity.type}</div>)}
      </div>
      </div>
    </NavbarDesktop>
  );
